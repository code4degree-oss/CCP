  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <WizSectionHeader icon={GraduationCap} title={isEng ? 'Examination Details' : 'NEET-UG Details'} color="text-blue-700" />
        <div className="grid grid-cols-2 gap-4">
          {isEng ? (<>
            <div className="col-span-2 border-b border-gray-100 pb-2 mb-2"><h4 className="text-sm font-bold text-gray-700">MHT-CET Details</h4></div>
            <Field label="MHT-CET Roll Number" half><input value={p2.cet_roll_no} onChange={e => s('cet_roll_no', e.target.value)} className={inputClass} /></Field>
            <Field label="MHT-CET Application Number" half><input value={p2.cet_application_no} onChange={e => s('cet_application_no', e.target.value)} className={inputClass} /></Field>
            <Field label="PCM Marks/Score" half><input type="number" step="0.01" value={p2.cet_score} onChange={e => s('cet_score', e.target.value)} className={inputClass} /></Field>
            <Field label="Date of Birth (Exam Verification)" half><input type="date" value={p2.cet_dob} onChange={e => s('cet_dob', e.target.value)} className={inputClass} /></Field>
            
            <div className="col-span-2 border-b border-gray-100 pb-2 mt-4 mb-2"><h4 className="text-sm font-bold text-gray-700">JEE Details</h4></div>
            ...
            <Field label="Date of Birth (Exam Verification)" half><input type="date" value={p2.dob} onChange={e => s('dob', e.target.value)} className={inputClass} /></Field>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Full Name as per HSC Marksheet" required half><input value={p2.full_name} onChange={e => s('full_name', e.target.value)} className={inputClass} /></Field>
          ...
          {!isEng && <Field label="APAAR ID" half><input value={p2.apaar_id} onChange={e => s('apaar_id', e.target.value)} className={inputClass} /></Field>}
          ...
          <Field label="Mother Tongue" half><input value={p2.mother_tongue} onChange={e => s('mother_tongue', e.target.value)} className={inputClass} /></Field>
          ...
          <Field label="Region Classification" half><select value={p2.region_of_residence} onChange={e => s('region_of_residence', e.target.value)} className={selectClass}><option value="">Select</option><option>Mahanagar Palika</option><option>Nagar Palika</option><option>Panchayat Samiti</option><option>Gram Panchayat</option></select></Field>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Village" half><input value={p2.village} onChange={e => s('village', e.target.value)} className={inputClass} /></Field>
        ...
        <div className="mt-4 flex items-center gap-2">
          <input type="checkbox" id="addresses_different" checked={p2.addresses_different} onChange={e => s('addresses_different', e.target.checked)} className="w-4 h-4 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500" />
          <label htmlFor="addresses_different" className="text-sm font-medium text-gray-700">Present Address and Permanent Address are Different</label>
        </div>
        
        {p2.addresses_different && (
          <div className="mt-6 pt-6 border-t border-gray-100">
            <h4 className="text-sm font-bold text-gray-800 mb-4 uppercase tracking-wider">Present Address</h4>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Address Line 1" half><input value={p2.present_address_line1} onChange={e => s('present_address_line1', e.target.value)} className={inputClass} /></Field>
              <Field label="Address Line 2" half><input value={p2.present_address_line2} onChange={e => s('present_address_line2', e.target.value)} className={inputClass} /></Field>
              <Field label="Address Line 3" half><input value={p2.present_address_line3} onChange={e => s('present_address_line3', e.target.value)} className={inputClass} /></Field>
              <Field label="Village" half><input value={p2.present_village} onChange={e => s('present_village', e.target.value)} className={inputClass} /></Field>
              <Field label="City" half><input value={p2.present_city} onChange={e => s('present_city', e.target.value)} className={inputClass} /></Field>
              <Field label="State" half><SearchSelect value={p2.present_state} onChange={v => { s('present_state', v); s('present_district', ''); s('present_taluka', '') }} options={[...STATES]} placeholder="Select State" /></Field>
              <Field label="District" half>{p2.present_state === 'Other' ? <input value={p2.present_district} onChange={e => { s('present_district', e.target.value); s('present_taluka', '') }} placeholder="Enter District" className={inputClass} /> : <SearchSelect value={p2.present_district} onChange={v => { s('present_district', v); s('present_taluka', '') }} options={getDistricts(p2.present_state)} placeholder="Select District" disabled={!p2.present_state} />}</Field>
              <Field label="Taluka" half>{p2.present_state === 'Other' ? <input value={p2.present_taluka} onChange={e => s('present_taluka', e.target.value)} placeholder="Enter Taluka" className={inputClass} /> : <SearchSelect value={p2.present_taluka} onChange={v => s('present_taluka', v)} options={getTalukas(p2.present_state, p2.present_district)} placeholder="Select Taluka" disabled={!p2.present_district} />}</Field>
              <Field label="Pin Code" half><input value={p2.present_pincode} onChange={e => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 6) s('present_pincode', v); }} className={inputClass} /></Field>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Category of Candidate" half><select value={p2.category_of_candidate} onChange={e => s('category_of_candidate', e.target.value)} className={selectClass}><option value="">Select</option><option>OPEN</option><option>SC</option><option>ST</option><option>EWS</option><option>OBC</option><option>SEBC</option><option>SBC</option><option>NT1</option><option>NT2</option><option>NT3</option><option>VJDT</option></select></Field>
          ...
          <Field label="Annual Family Income" half><select value={p2.annual_income} onChange={e => s('annual_income', e.target.value)} className={selectClass}><option value="">Select</option><option>Below 1 Lakh</option><option>1-2.5 Lakh</option><option>2.5-5 Lakh</option><option>5-8 Lakh</option><option>Above 8 Lakh</option></select></Field>
        </div>

        {/* Dynamic Document Sections */}
        {p2.category_of_candidate && p2.category_of_candidate !== 'OPEN' && (
          <div className="col-span-2 mt-6 space-y-6 border-y border-gray-100 py-6">
            <h4 className="text-sm font-bold text-gray-800 uppercase tracking-wider mb-2">Category Document Details</h4>
            
            {/* EWS Only */}
            {p2.category_of_candidate === 'EWS' && (
              <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100">
                <Field label="Do you possess EWS Certificate?"><select value={p2.ews_cert_status} onChange={e => s('ews_cert_status', e.target.value)} className={selectClass}><option value="">Select</option><option>AVAILABLE</option><option>APPLIED</option><option>NOT AVAILABLE</option></select></Field>
                {p2.ews_cert_status === 'AVAILABLE' && (
                  <div className="grid grid-cols-3 gap-4 mt-3">
                    <Field label="Document Number"><input value={p2.ews_cert_doc_no} onChange={e => s('ews_cert_doc_no', e.target.value)} className={inputClass} /></Field>
                    <Field label="Issuing Date"><input type="date" value={p2.ews_cert_issue_date} onChange={e => s('ews_cert_issue_date', e.target.value)} className={inputClass} /></Field>
                    <Field label="Issued District"><input value={p2.ews_cert_district} onChange={e => s('ews_cert_district', e.target.value)} className={inputClass} /></Field>
                  </div>
                )}
                {p2.ews_cert_status === 'APPLIED' && (
                  <div className="grid grid-cols-3 gap-4 mt-3">
                    <Field label="Receipt Number"><input value={p2.ews_cert_receipt_no} onChange={e => s('ews_cert_receipt_no', e.target.value)} className={inputClass} /></Field>
                    <Field label="Applied Date"><input type="date" value={p2.ews_cert_applied_date} onChange={e => s('ews_cert_applied_date', e.target.value)} className={inputClass} /></Field>
                    <Field label="Application District"><input value={p2.ews_cert_app_district} onChange={e => s('ews_cert_app_district', e.target.value)} className={inputClass} /></Field>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Other Categories */}
            {['SC', 'ST', 'OBC', 'SEBC', 'SBC', 'NT1', 'NT2', 'NT3', 'VJDT'].includes(p2.category_of_candidate) && (
              <>
                <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100">
                  <Field label="Do you possess Caste Certificate?"><select value={p2.caste_cert_status} onChange={e => s('caste_cert_status', e.target.value)} className={selectClass}><option value="">Select</option><option>AVAILABLE</option><option>APPLIED</option><option>NOT AVAILABLE</option></select></Field>
                  {p2.caste_cert_status === 'AVAILABLE' && (
                    <div className="grid grid-cols-3 gap-4 mt-3">
                      <Field label="Document Number"><input value={p2.caste_cert_doc_no} onChange={e => s('caste_cert_doc_no', e.target.value)} className={inputClass} /></Field>
                      <Field label="Issuing Date"><input type="date" value={p2.caste_cert_issue_date} onChange={e => s('caste_cert_issue_date', e.target.value)} className={inputClass} /></Field>
                      <Field label="Issued District"><input value={p2.caste_cert_district} onChange={e => s('caste_cert_district', e.target.value)} className={inputClass} /></Field>
                    </div>
                  )}
                  {p2.caste_cert_status === 'APPLIED' && (
                    <div className="grid grid-cols-3 gap-4 mt-3">
                      <Field label="Receipt Number"><input value={p2.caste_cert_receipt_no} onChange={e => s('caste_cert_receipt_no', e.target.value)} className={inputClass} /></Field>
                      <Field label="Applied Date"><input type="date" value={p2.caste_cert_applied_date} onChange={e => s('caste_cert_applied_date', e.target.value)} className={inputClass} /></Field>
                      <Field label="Application District"><input value={p2.caste_cert_app_district} onChange={e => s('caste_cert_app_district', e.target.value)} className={inputClass} /></Field>
                    </div>
                  )}
                </div>
                
                <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100">
                  <Field label="Do you possess Caste Validity?"><select value={p2.caste_validity_status} onChange={e => s('caste_validity_status', e.target.value)} className={selectClass}><option value="">Select</option><option>AVAILABLE</option><option>APPLIED</option><option>NOT AVAILABLE</option></select></Field>
                  {p2.caste_validity_status === 'AVAILABLE' && (
                    <div className="grid grid-cols-3 gap-4 mt-3">
                      <Field label="Document Number"><input value={p2.caste_validity_doc_no} onChange={e => s('caste_validity_doc_no', e.target.value)} className={inputClass} /></Field>
                      <Field label="Issuing Date"><input type="date" value={p2.caste_validity_issue_date} onChange={e => s('caste_validity_issue_date', e.target.value)} className={inputClass} /></Field>
                      <Field label="Issued District"><input value={p2.caste_validity_district} onChange={e => s('caste_validity_district', e.target.value)} className={inputClass} /></Field>
                    </div>
                  )}
                  {p2.caste_validity_status === 'APPLIED' && (
                    <div className="grid grid-cols-3 gap-4 mt-3">
                      <Field label="Receipt Number"><input value={p2.caste_validity_receipt_no} onChange={e => s('caste_validity_receipt_no', e.target.value)} className={inputClass} /></Field>
                      <Field label="Applied Date"><input type="date" value={p2.caste_validity_applied_date} onChange={e => s('caste_validity_applied_date', e.target.value)} className={inputClass} /></Field>
                      <Field label="Application District"><input value={p2.caste_validity_app_district} onChange={e => s('caste_validity_app_district', e.target.value)} className={inputClass} /></Field>
                    </div>
                  )}
                </div>
              </>
            ...
            {/* NCL - Not for SC/ST */}
            {['OBC', 'SEBC', 'SBC', 'NT1', 'NT2', 'NT3', 'VJDT'].includes(p2.category_of_candidate) && (
              <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100">
                <Field label="Do you possess NCL Certificate?"><select value={p2.ncl_cert_status} onChange={e => s('ncl_cert_status', e.target.value)} className={selectClass}><option value="">Select</option><option>AVAILABLE</option><option>APPLIED</option><option>NOT AVAILABLE</option></select></Field>
                {p2.ncl_cert_status === 'AVAILABLE' && (
                  <div className="grid grid-cols-3 gap-4 mt-3">
                    <Field label="Document Number"><input value={p2.ncl_cert_doc_no} onChange={e => s('ncl_cert_doc_no', e.target.value)} className={inputClass} /></Field>
                    <Field label="Issuing Date"><input type="date" value={p2.ncl_cert_issue_date} onChange={e => s('ncl_cert_issue_date', e.target.value)} className={inputClass} /></Field>
                    <Field label="Issued District"><input value={p2.ncl_cert_district} onChange={e => s('ncl_cert_district', e.target.value)} className={inputClass} /></Field>
                  </div>
                )}
                {p2.ncl_cert_status === 'APPLIED' && (
                  <div className="grid grid-cols-3 gap-4 mt-3">
                    <Field label="Receipt Number"><input value={p2.ncl_cert_receipt_no} onChange={e => s('ncl_cert_receipt_no', e.target.value)} className={inputClass} /></Field>
                    <Field label="Applied Date"><input type="date" value={p2.ncl_cert_applied_date} onChange={e => s('ncl_cert_applied_date', e.target.value)} className={inputClass} /></Field>
                    <Field label="Application District"><input value={p2.ncl_cert_app_district} onChange={e => s('ncl_cert_app_district', e.target.value)} className={inputClass} /></Field>
                  </div>
                )}
              </div>
            )}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="SSC Seat Number" half><input value={p2.ssc_roll_no} onChange={e => s('ssc_roll_no', e.target.value)} className={inputClass} /></Field>
          <Field label="Language / Medium" half><select value={p2.ssc_language} onChange={e => s('ssc_language', e.target.value)} className={selectClass}><option value="">Select</option><option>Urdu</option><option>Arabic</option><option>Persian</option><option>Null</option></select></Field>
          ...
          <Field label="Passing District" half>{p2.ssc_state === 'Other' ? <input value={p2.ssc_district} onChange={e => { s('ssc_district', e.target.value); s('ssc_taluka', '') }} className={inputClass} /> : <SearchSelect value={p2.ssc_district} onChange={v => { s('ssc_district', v); s('ssc_taluka', '') }} options={getDistricts(p2.ssc_state)} placeholder="Select District" disabled={!p2.ssc_state} />}</Field>
          <Field label="Passing Taluka" half>{p2.ssc_state === 'Other' ? <input value={p2.ssc_taluka} onChange={e => s('ssc_taluka', e.target.value)} className={inputClass} /> : <SearchSelect value={p2.ssc_taluka} onChange={v => s('ssc_taluka', v)} options={getTalukas(p2.ssc_state, p2.ssc_district)} placeholder="Select Taluka" disabled={!p2.ssc_district} />}</Field>
        </div>
        
        <div className="mt-6 border-t border-gray-100 pt-6">
          <h4 className="text-sm font-bold text-gray-800 mb-4 uppercase tracking-wider">Subject-wise Marks</h4>
          <div className="grid grid-cols-3 gap-4">
            <Field label="Mathematics"><input type="number" value={p2.ssc_maths} onChange={e => s('ssc_maths', e.target.value)} className={inputClass} /></Field>
            <Field label="Science"><input type="number" value={p2.ssc_science} onChange={e => s('ssc_science', e.target.value)} className={inputClass} /></Field>
            <Field label="English"><input type="number" value={p2.ssc_english} onChange={e => s('ssc_english', e.target.value)} className={inputClass} /></Field>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Mother's Name as per HSC Marksheet" half><input value={p2.hsc_mother_name} onChange={e => s('hsc_mother_name', e.target.value)} className={inputClass} /></Field>
          <Field label="Examination Status" half><select value={p2.hsc_exam_status} onChange={e => s('hsc_exam_status', e.target.value)} className={selectClass}><option value="">Select</option><option>Pass</option><option>Fail</option></select></Field>
          ...
          <Field label="Roll No. of 12th" half><input value={p2.hsc_roll_no} onChange={e => s('hsc_roll_no', e.target.value)} className={inputClass} /></Field>
          ...
          <Field label="Exam Session" half><select value={p2.hsc_exam_session} onChange={e => s('hsc_exam_session', e.target.value)} className={selectClass}><option value="">Select</option><option>March</option><option>October</option></select></Field>
          <Field label="Board Name" half><select value={p2.hsc_exam} onChange={e => s('hsc_exam', e.target.value)} className={selectClass}><option value="">Select Board</option><option>HSC</option><option>CBSE</option><option>ICSE (CISCE)</option><option>NIOS</option><option>IB (International Baccalaureate)</option><option>Cambridge (IGCSE)</option><option>Maharashtra State Board</option><option>Andhra Pradesh Board</option><option>Assam Board (SEBA)</option><option>Bihar Board (BSEB)</option><option>Chhattisgarh Board (CGBSE)</option><option>Goa Board (GBSHSE)</option><option>Gujarat Board (GSEB)</option><option>Haryana Board (BSEH)</option><option>Himachal Pradesh Board (HPBOSE)</option><option>Jharkhand Board (JAC)</option><option>Karnataka Board (KSEEB)</option><option>Kerala Board (KBPE)</option><option>Madhya Pradesh Board (MPBSE)</option><option>Manipur Board (BSEM)</option><option>Meghalaya Board (MBOSE)</option><option>Mizoram Board (MBSE)</option><option>Nagaland Board (NBSE)</option><option>Odisha Board (BSE)</option><option>Punjab Board (PSEB)</option><option>Rajasthan Board (RBSE)</option><option>Tamil Nadu Board</option><option>Telangana Board (BSETS)</option><option>Tripura Board (TBSE)</option><option>Uttar Pradesh Board (UPMSP)</option><option>Uttarakhand Board (UBSE)</option><option>West Bengal Board (WBBSE)</option><option>J&K Board (JKBOSE)</option><option>Other</option></select></Field>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-[1fr_1fr_1fr] gap-4 items-end border-t border-gray-100 pt-4">
            <div><p className="text-sm font-semibold text-gray-800">Highest Marks in Subject</p></div>
            <input type="number" value={p2.highest_marks_subject} onChange={e => {
              const v = e.target.value.replace(/\D/g, ''); if(v.length <= 3) s('highest_marks_subject', v);
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Claim Exception?" half><select value={p2.claim_exception} onChange={e => { s('claim_exception', e.target.value); if (e.target.value === 'None' || !e.target.value) s('specified_reservation', '') }} className={selectClass}><option value="">Select</option><option>4.7</option><option>4.8</option><option>None</option></select></Field>
          {p2.claim_exception && p2.claim_exception !== 'None' && (
            <Field label="Specified Reservation" half>
              <div className="space-y-2 pt-1">{['D1', 'D2', 'D3', 'MKB', 'Hilly Area', 'None'].map(opt => (
